...
class IEEAppsAPIConnector extends Controller
{
    ...
    function callback_join_personal_info_filler_iee_apps(Request $request){
        ...
        if($request->session()->has('api_action_type') && $request->session()->has('return_url')){
            ...
            if($request->session()->get('api_action_type') === "join_person_info_filler_callback"){
            ...
            if($request->session()->get('login_filler_callback') === "join_person_info_filler_callback"){
                return view('login');
            }
    ...
    function login_filler_iee_apps(Request $request){
        $request->session()->put('return_url', url()->previous());
        $request->session()->put('api_action_type', 'login_filler_callback');
